...
**보안과 확장성을 모두 고려한 개발**을 지향하며, 안전한 서비스 개발은 저의 숙명이자 책임입니다.<br>
Secure by Design 원칙 아래에 **사용자가 믿고 안심할 수 있는 서비스**를 만드는 데에 자신있습니다.<br><br>
어떤 문제든 **근본적인 원인**을 파악하고, 동료들의 의도를 이해하며 최상의 해결책을 찾습니다.<br>
...
* 세부 업무
    ...
    * 별도의 계정이 필요하지 않은 서버, Web/WAS, Docker 스크립트 통합(실행 횟수 3회 → 1회로 감소) 
    * 반복적인 커뮤니케이션을 제거하여 진단 프로세스 간소화 및 결과 수집 시간 50% 단축
    * 자동화 도구를 확장 가능하도록 설계(설정 값 추출, 결과 출력 분리)하여 유지보수 최소화
...
* 세부 업무
    ...
    - RSS Feed(구글 알리미, 보안뉴스, 데일리시큐), 네이버 검색 API를 이용해 뉴스 크롤링 시스템 개발
